...
// انتخاب جمعی برای هر لیست: گزینهٔ «انتخاب» در منوی لمسی + نوار عملیات گروهی
// نحوهٔ استفاده: const b = useBulkSelect(ids, async sel => {...حذف گروهی...});
// روی هر ActionRow: selectMode={b.mode} selected={b.has(id)} onToggle={()=>b.toggle(id)}
// و در ابتدای آرایهٔ actions: b.menuAction(id)  — در انتهای صفحه: {b.bar}
export function useBulkSelect(allIds: string[], onDelete: (ids: string[]) => Promise<void> | void, opts: { label?: string; deleteLabel?: string } = {}) {
  const [mode, setMode] = useState(false);
  const [sel, setSel] = useState<Set<string>>(new Set());
  const label = opts.label || 'مورد';
  const start = (id: string) => { setMode(true); setSel(new Set([id])); };
  const toggle = (id: string) => setSel(p => { const n = new Set(p); if (n.has(id)) n.delete(id); else n.add(id); return n; });
  const exit = () => { setMode(false); setSel(new Set()); };
  const all = () => setSel(new Set(allIds));
  async function doDelete() {
    if (!sel.size) return;
    if (!(await confirmDialog({ title: `حذف ${sel.size.toLocaleString('fa-IR')} ${label}؟`, message: 'این عملیات قابل بازگشت نیست.', danger: true }))) return;
    await onDelete([...sel]);
    exit();
  }
  const menuAction = (id: string): MenuAction => ({ label: 'انتخاب (عملیات گروهی)', onClick: () => start(id) });
  const bar = mode ? (
    <div className="fixed bottom-20 left-1/2 z-40 flex -translate-x-1/2 items-center gap-2 rounded-2xl bg-zinc-950 px-3 py-2.5 text-white shadow-2xl border border-white/10">
      <span className="text-[11px] whitespace-nowrap">{sel.size.toLocaleString('fa-IR')} {label}</span>
      <button onClick={all} className="rounded-xl bg-zinc-800 px-2.5 py-1.5 text-[10px] font-bold whitespace-nowrap">انتخاب همه</button>
      <button onClick={() => void doDelete()} disabled={!sel.size} className="rounded-xl bg-red-500 px-2.5 py-1.5 text-[10px] font-bold disabled:opacity-40 whitespace-nowrap">{opts.deleteLabel || 'حذف گروهی'}</button>
      <button onClick={exit} className="rounded-xl bg-zinc-800 px-2.5 py-1.5 text-[10px] whitespace-nowrap">انصراف</button>
    </div>
  ) : null;
  return { mode, sel, has: (id: string) => sel.has(id), start, toggle, exit, menuAction, bar };
}
